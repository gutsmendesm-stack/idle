const API_BASE_URL = 'LOCAL_BYPASS';
...
async function assertExtensionUpToDate() { return; }
...
function isVipForProduct(contaStatus, productId) { return true; }
...
async function fetchModuleAccessToken(opts) {
  // PATCHED: No token needed
  return 'local-bypass';
}

async function fetchModuleCode(moduleName) {
  // PATCHED: Load from local files instead of API
